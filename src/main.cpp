...
/* Requirements:
  ...
  http://arduino.esp8266.com/stable/package_esp8266com_index.json Time 1.5.0
  https://github.com/PaulStoffregen/Time SimpleTimer
  https://github.com/jfturcot/SimpleTimer
  (http://playground.arduino.cc/Code/SimpleTimer) NtpClientLib 2.0.5
  https://github.com/gmag11/NtpClient ArduinoJson 5.6.7
  https://github.com/bblanchon/ArduinoJson
  (https://bblanchon.github.io/ArduinoJson/) U8G2Lib 2.13.5
  https://github.com/olikraus/u8g2 orbitalair-arduino-rtc-pcf8563
  ...
  This uses the SPIFFS file system.  So we need to load that in your esp-xx
  first. Upload the contents of the data folder with MkSPIFFS Tool ("ESP8266
  Sketch Data Upload" in Tools menu in Arduino IDE)
...
char ssid[31] = {"12345678"}; // This is the access point to connect to
#define EEPROM_SSID 0         // where the ssid is kept in EEPROM
char password[31] = {"nyesteferenc12345678"}; // And its password
#define EEPROM_PASSWORD 32    // where the password is kept in EEPROM
char host[31] = {"ontozes"};  // The host name .local (mdns) hostname
#define EEPROM_HOST 64        // where the host is kept in EEPROM
char assid[31] = {"ontozes"}; // This is the access point to connect to
char apassword[31] = {"nyesteferenc12345678"}; // And its password

int offsetGMT = 0;                   // Local timezone offset in seconds
char offsetGMTstring[10] = {"7200"}; // String version of it
...
const uint8_t STCP = 15; // D8, storage register clock, latch_pin
...
const char *_pretty_days(int d) {
static char pd_buf[9] = {'\0'};
...
void _print_sched(struct _sched *s) {
DBG_OUTPUT_PORT.printf(
    "z:%3d d:%8s start:%4ld stop:%4ld, b:%8ld e:%8ld, n:%8ld\n", s->zone,
    _pretty_days(s->days), s->start / 60, s->stop / 60, s->begin, s->end,
    s->next);
}

#ifdef EXTRA_DEBUG
/** Pretty print schedule to debug info.
*/
void _print_schedules() {
File json = LittleFS.open(schedFile, "r");
DynamicJsonDocument root(2048);
DeserializationError _error = deserializeJson(root, json);
if (_error) {
  DBG_OUTPUT_PORT.printf("Error reading scheduler file\n%s\n",
                         _error.c_str());
  return;
}
serializeJsonPretty(root, DBG_OUTPUT_PORT);
}
#endif
...
// displayStatus(1, "HTTP Update Failed");
  ...
  if (Update.end(true)) {
    DBG_OUTPUT_PORT.printf("Update Success: %ld\nRebooting...\n",
                           update.totalSize);
...
// displayStatus(2, "upload %s", filename.c_str());
  ...
  if (fsUploadFile)
    fsUploadFile.close();
  DBG_OUTPUT_PORT.printf("handleFileUpload Size: %ld\n", upload.totalSize);
...
if (LittleFS.exists(configFile)) {
  ...
  if (_error) {
    ...
    t = root["timeServer"];
    if (t) {
      strncpy(timeServer, t, 30);
    }
    t = root["offsetGMT"];
    if (t) {
      strncpy(offsetGMTstring, t, 10);
    }
...
DBG_OUTPUT_PORT.printf(
    "%s: %08x z:%d s:%ld d:%02x b:%ld e:%ld n:%ld o:%ld d:%d\n", m, s,
    s->zone, s->start, s->days, s->begin, s->end, s->next, s->stop,
    s->duration);
DBG_OUTPUT_PORT.printf("%s: next:%s", m, timeStr(s->next + offsetGMT));
DBG_OUTPUT_PORT.printf(" stop:%s\n", timeStr(s->stop + offsetGMT));
}
#endif

void xxdoRestart() { ESP.restart(); }
...
void loadSched() {
if (LittleFS.exists(schedFile)) {
  File file = LittleFS.open(schedFile, "r");
  char json[1000];
  memset(json, 0, sizeof(json));
  file.readBytes(json, sizeof(json));
  file.close();
  ...
  if (_error) {
    ...
    for (int i = 0; i < MAX_SCHED; i++) {
      ...
      sched[i].duration = 0;
      sched[i].stop = 0;
      if (zoneString && zoneString[0] != '\0' && startTime &&
          startTime[0] != '\0') {
        //
        ...
        sched[i].start = atoi(startTime) * 3600 + atoi(startTime + 3) * 60;
        ...
        int duration = root[i]["duration"];
        DBG_OUTPUT_PORT.printf("DURATION --> %d\n",
                               root[i]["duration"].as<int>());
        sched[i].duration = (duration == 0) ? 15 : duration;
        sched[i].duration *= 60;
...
DBG_OUTPUT_PORT.printf("%s set relays=%s\n", timstamp(),
                       _pretty_days(relayState));
digitalWrite(STCP, LOW);
shiftOut(DS, SHCP, MSBFIRST, relayState ^ 0xFF);
...
int setTimedFunc(bool repeat, int *id, sint32_t t, void (*func)(),
               const char *name) {
if (*id > -1) {
#ifdef EXTRA_DEBUG
  DBG_OUTPUT_PORT.printf("clearingTimer id=%d\n", *id);
...
if (event) {
  ...
  timer.setTimeout(2000, recalcSched);
  if (hasRtc && !rtcValid) {
...
tmElements_t tm;
breakTime(t, tm);
snprintf(tmbuf, sizeof(tmbuf), "%02d:%02d:%02d %04d/%02d/%02d", tm.Hour,
         tm.Minute, tm.Second, tm.Year + 1970, tm.Month, tm.Day);
return tmbuf;
}

const PROGMEM char *wdays[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
const PROGMEM char *mons[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
...
if (host[0] == '*' && host[1] == '\0') {
  ...
  snprintf(host, sizeof(host), "ESPrinkler2_%02x%02x%02x", mac[3], mac[4],
           mac[5]);
}
}
...
DBG_OUTPUT_PORT.printf("Setting up display.\n");
// u8g2.begin();
...
DBG_OUTPUT_PORT.printf("Setting up EEPROM\n");
...
DBG_OUTPUT_PORT.printf("Setting up LittleFS\n");
if (LittleFS.begin())
{
  ...
  DBG_OUTPUT_PORT.println("Cannot open LittleFS filesystem\n");
}

DBG_OUTPUT_PORT.printf("Loading config.\n");
loadConfig();
...
// displayStatus(0, "RTC found");
  ...
  DBG_OUTPUT_PORT.printf(
      ...
      rtcValid ? "" : "not ", dt.Hour(), dt.Minute(), dt.Second(), dt.Year(),
      dt.Month(), dt.Day());
  if (rtcValid) {
    setTime(dt.Hour(), dt.Minute(), dt.Second(), dt.Day(), dt.Month(),
            dt.Year());
  }
}
#endif
...
// displayStatus(0, "RTC found");
  ...
  uint8_t stat1 = Wire.read();
  uint8_t stat2 = Wire.read();
  uint8_t stat3 = Wire.read();
...
// displayStatus(0, "RTC not valid");
  ...
  DBG_OUTPUT_PORT.printf("RTC PCF8563 time is %svalid: %s %s\n",
                         rtcValid ? "" : "not ",
                         Rtc_Pcf8563.formatDate(RTCC_DATE_ASIA),
...
server.on("/update", HTTP_POST, afterFileUpdate, handleFileUpdate);
server.on("/spiffsupdate", HTTP_POST, afterFileUpdate, handleFileUpdate);

server.on("/status", HTTP_GET, []() {
// displayStatus(2, "%s", server.uri().c_str());
  String json = "{";
  for (int i = 0; i < 8; i++) {
    json += String("\"zone") + String(i) +
...
// displayStatus(2, "%s", server.uri().c_str());
  ...
  LittleFS.remove(schedFile);
  LittleFS.remove(buttonsFile);
  server.sendHeader("Cache-Control", "public, max-age=0");
...
ArduinoOTA.onError([](ota_error_t error) {
  ...
  if (error == OTA_AUTH_ERROR)
    DBG_OUTPUT_PORT.printf("Auth Failed\n");
  else if (error == OTA_BEGIN_ERROR)
    DBG_OUTPUT_PORT.printf("Begin Failed\n");
  else if (error == OTA_CONNECT_ERROR)
    DBG_OUTPUT_PORT.printf("Connect Failed\n");
  else if (error == OTA_RECEIVE_ERROR)
    DBG_OUTPUT_PORT.printf("Receive Failed\n");
  else if (error == OTA_END_ERROR)
    DBG_OUTPUT_PORT.printf("End Failed\n");
