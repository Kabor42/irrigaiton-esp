...
void _print_sched(struct _sched *s) {
  DBG_OUTPUT_PORT.printf(
      ...
      s->zone, _pretty_days(s->days), s->start / 60, s->stop / 60, s->begin,
      s->end, s->next);
}
...
void dumpSched(const char *m, struct _sched *s) {
  DBG_OUTPUT_PORT.printf("%s: %08x z:%d s:%d d:%02x b:%d e:%d n:%d o:%d d:%d\n",
...
void onSTAGotIP(WiFiEventStationModeGotIP ipInfo) {
  ...
  displayStatus(0, "STA:%s %s %d", WiFi.localIP().toString().c_str(), ssid,
                WiFi.RSSI());
  setApModeTimeout(0);
  setBlinker(500);
